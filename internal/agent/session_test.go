...
func TestForkRestoresLeafOnModelRestoreFailure(t *testing.T) {
	t.Parallel()
	...
	if err := store.AppendMessage(textMessage(agentcore.RoleUser, "m1")); err != nil {
		t.Fatalf("append message 1: %v", err)
	}
	if err := store.AppendModelChange("openai", "bad-model"); err != nil {
		t.Fatalf("append bad model change: %v", err)
	}
	if err := store.AppendMessage(textMessage(agentcore.RoleAssistant, "m2")); err != nil {
	...
	for _, e := range entries {
		if e.Kind == session.EntryModelChange {
			targetID = e.ID
			break
		}
	}
	if targetID == "" {
		t.Fatalf("missing model_change entry")
	}
	...
	if !strings.Contains(err.Error(), "restore model after fork") {
		t.Fatalf("unexpected error: %v", err)
	}

	if got := store.LeafID(); got != prevLeaf {
		t.Fatalf("leaf should rollback after fork failure: got %s want %s", got, prevLeaf)
	}
}
