...
func buildRuntime(input *bootInput, spec *bootSpec) (*Runtime, error) {
	...
	return &Runtime{
		...
		PlanPhase:           input.snapshot.PlanPhase,
		PlanPreMode:         input.snapshot.PlanPreMode,
		PlanAllowedCommands: append([]storage.AllowedCommandEntry(nil), input.snapshot.PlanAllowedCommands...),
	}, nil
}
